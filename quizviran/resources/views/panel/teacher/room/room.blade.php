          ...
          <div class="col-12">
            <div class="mt-3">
              <app-room-comments
                  ...
                  route="{{ route('quizviran.comment.paginate',['room' => $room->link]) }}"
                  userType="{{ $user->type }}"
              >
              </app-room-comments>
            </div>
                ...
                @foreach($room->exams as $exam)
                  <div class="text-center row flex-column justify-content-center align-items-center">
                    ...
                    <div class="d-flex w-100 flex-row flex-wrap rounded overflow-hidden figure-caption bg-dark-gray">
                      ...
                      <span class="p-2 col-6 col-lg-12">{{ jalaly($exam->start)->format('H:i m/d') }}</span>
                      <div class="divider"></div>
                      <span class="p-2 col-6 col-lg-12">مدت زمان آزمون</span>
                      <span class="p-2 col-6 col-lg-12">{{ $exam->duration }} دقیقه </span>
                      @if($user->isTeacher() || (!$user->isTeacher() && $exam->isInTime()))
